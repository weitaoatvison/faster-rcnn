    ...
    for(size_t i = 0; i < imgs.size(); i++)
    {
        ...
        for(int k = 0; k < sample.channels(); k++)
        {
            for(int i = 0; i < sample.rows; i++)
            {
                ...
                {
                    input_data[cnt] = float(sample.at<uchar>(i,j*3+k)) - pixel_means_[k];
                    cnt += 1;
                }
            }
        }
...
void Faster_rcnn::get_detection(vector<Blob<float>* >& outputs, vector<struct Bbox> &final_vbbox)
{
