...
WORK_DIR='/home/radmin/dl/data/kitti'
...
def _write_voc_results_file(all_boxes,__image_index):
    for cls_ind, cls in enumerate(CLASSES):
        ...
        filename=WORK_DIR+'/result/comp4_'+cls
...
def _do_python_eval(output_dir='output'):
    ...
    annopath=os.path.join(WORK_DIR,'training','label_2','{:s}.txt')
    imagesetfile=os.path.join(WORK_DIR,'training','image_2','train.txt')

    cachedir=os.path.join(WORK_DIR,'annotations_cache')
...
def _load_image_set_index():
    ...
    image_set_file=os.path.join(WORK_DIR,'training','image_2','train.txt')
...
def image_path_from_index(index):
    ...
    """
    image_path=os.path.join(WORK_DIR,'training','image_2',index+'.jpg')
...
def test_net(net, max_per_image=100, thresh=0.05, vis=False):
    ...
    output_dir=os.path.join(WORK_DIR,'running_data')
    # timers
    ...
    for i in xrange(num_images):
        ...
        image_path=os.path.join(WORK_DIR,'training','image_2',image_index[i]+'.png')
        assert os.path.exists(image_path), \
            'Path does not exist: {}'.format(image_path)
...
if __name__ == '__main__':
    ...
    prototxt=os.path.join(WORK_DIR,'faster_rcnn_end2end','test.prototxt')
    caffemodel=os.path.join(WORK_DIR,'faster_rcnn_models','vgg16_faster_rcnn_iter_70000_kitti.caffemodel')

    Input_Path=os.path.join(WORK_DIR,'testing','image_2')
    Out_Path=os.path.join(WORK_DIR,'testing','label_2')
